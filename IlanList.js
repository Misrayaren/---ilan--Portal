...
const is_ilanlari_veriler = [
  ...
  const IlanKartlari = ({ ilan, onIlanClick }) => (
    <div className="ilan" onClick={() => onIlanClick(ilan)}>
  ...
  const IlanList = ({ searchTerm, searchTriggered }) => {
    ...
    if (selectedIlan) {
      return (
        <div className="ilan-detay">
          <h2>{selectedIlan.job_title}</h2>
          <img src={selectedIlan.job_image_url} alt={selectedIlan.job_title} />
          <p><strong>{selectedIlan.job_description}</strong></p>
          <p><i>Konum:</i> {selectedIlan.location}</p>
          <p><i>Kategori:</i> {selectedIlan.category}</p>
          <button onClick={() => setSelectedIlan(null)}>Geri</button>
        </div>
      );
    }
  
    // Only filter the listings if the search button was pressed
    const filteredIlanlar = searchTriggered
      ? is_ilanlari_veriler.filter(ilan => 
      ...
      <div id="ilan-listesi">
        {filteredIlanlar.length > 0 ? (
          filteredIlanlar.map((ilan, index) => (
            <IlanKartlari key={index} ilan={ilan} onIlanClick={handleIlanClick} />
          ))
        ) : (
